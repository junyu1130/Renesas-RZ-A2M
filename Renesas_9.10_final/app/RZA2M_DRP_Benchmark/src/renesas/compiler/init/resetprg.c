...
******************************************************************************/
static void r_disable_rtc(uint32_t ch);
static void r_disable_usb(uint32_t ch);

static st_r_drv_gpio_pin_list_t bsc_pin_list;

/* Init BSC to enable external SDRAM */
/* To Use SDRAM function on RSK board
 * 1. switch SW6-1 to ON
 * 2. enable STARTUP_CFG_SDRAM_ENABLE in r_startup_config.h
 * 3. CN5 UART will be disabled
 *  */
#if (STARTUP_CFG_SDRAM_ENABLE == 1)
static void sdram_init(void)
{
	int_t gpio_handle = (-1);
	st_r_drv_gpio_pin_list_t bsc_pin_list;

	//gpio driver requires FreeRTOS API, so have to init SDRAM after OS initialization
	//If system heap was
	gpio_handle = direct_open("gpio", 0);

	if ((-1) != gpio_handle)
	{
		bsc_pin_list.p_pin_list = &GPIO_SC_TABLE_BSC[0];
		bsc_pin_list.count = sizeof(GPIO_SC_TABLE_BSC) / sizeof(st_r_drv_gpio_sc_config_t);
		/* direct_control as direct_control assumes parameter 3 is i/o, but using the parameter in input only mode is acceptable */
		direct_control(gpio_handle, CTL_GPIO_INIT_BY_TABLE, (st_r_drv_gpio_pin_list_t *) &bsc_pin_list);
		direct_close(gpio_handle);
	}

	/* Initial setting of Bus State Controller for external SDRAM */
	R_BSC_Init(BSC_AREA_CS3);
}
#endif
...
void resetprg (void)
{
	...
	/* Some Driver initialization requires OS Heap support, add Heap Init here */
	R_OS_InitMemManager();
    ...
    /* Init Hyper bus controller to enable 8MB Hyper RAM on CPU board */
    R_HYPER_Init();
    ...
    /* Call to global objects initializer */
    __libc_init_array();
...
/*******************************************************************************
 End of function r_disable_rtc
 *******************************************************************************/
/******************************************************************************
* Function Name: r_disable_usb
...
static void r_disable_usb(uint32_t ch)
{
    ...
    static volatile struct st_usb00 * const H_USB00[R_PRV_USB_COUNT] =
    {
        &USB00, &USB10
    };
    /* USB Function IO reg Top Address(ch0, ch1) */
    static volatile struct st_usb01 * const F_USB01[R_PRV_USB_COUNT] =
    {
        &USB01, &USB11
    };
    /* MSTP */
    ...
    RZA_IO_RegWrite_32(&H_USB00[ch]->PHYCLK_CTRL.LONG, 0, USB_PHYCLK_CTRL_UCLKSEL_SHIFT, USB_PHYCLK_CTRL_UCLKSEL);

    /* 3: It can recover from deep standby by DP, DM change (PHYIF_CTRL.FIXPHY = 1) */
    RZA_IO_RegWrite_32(&H_USB00[ch]->PHYIF_CTRL.LONG, 1, USB_PHYIF_CTRL_FIXPHY_SHIFT, USB_PHYIF_CTRL_FIXPHY);

    /* 4: UTMI+PHY Normal Mode (LPSTS.SUSPM = 1) */
    RZA_IO_RegWrite_16(&F_USB01[ch]->LPSTS.WORD, 1, USB_LPSTS_SUSPM_SHIFT, USB_LPSTS_SUSPM);

    /* 5: UTMI + reset release (USBCTR.PLL_RST = 0) */
    RZA_IO_RegWrite_32(&H_USB00[ch]->USBCTR.LONG, 0, USB_USBCTR_PLL_RST_SHIFT, USB_USBCTR_PLL_RST);
    ...
    RZA_IO_RegWrite_32(&H_USB00[ch]->LINECTRL1.LONG, (USB_LINECTRL1_DPRPD_EN | USB_LINECTRL1_DMRPD_EN), IOREG_NONSHIFT_ACCESS, IOREG_NONMASK_ACCESS);

    /* 8: USBPHY standby mode (USBCTR.DIRPD = 1) */
    RZA_IO_RegWrite_32(&H_USB00[ch]->USBCTR.LONG, 1, USB_USBCTR_DIRPD_SHIFT, USB_USBCTR_DIRPD);
