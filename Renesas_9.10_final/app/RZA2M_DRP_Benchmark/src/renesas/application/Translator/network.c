...
* Return Value : no return value
***********************************************************************************************************************/
void convolution(TPrecision *dData,TPrecision *dPad, TPrecision *dWeights, TPrecision *dBiases,TPrecision *dOut,TsInt *iShapes){
	    	...
	    	for (iD3=0; iD3<iHp;iD3++)
	    	{
				...
				for(iItr1=0; iItr1<iC; iItr1++)
				{
					for(iItr2=ihs; iItr2<(ihs+iFH); iItr2++)
					{
						...
						{
							dvalue += dPad[(iD1*iC*iPH*iPW)+(iItr1*iPH*iPW)+(iItr2*iPW)+iItr3] * dWeights[(iD2*iC*iFH*iFW)+(iItr1*iFH*iFW)+((iItr2-ihs)*iFW)+(iItr3-iws)];
						}
					}
				}
...
* Return Value : no return value
***********************************************************************************************************************/
void innerproduct(TPrecision *data, TPrecision *weights, TPrecision *biases,TPrecision *out,TsInt *shapes){
		TsInt iRow, iColumn;
		TsInt iInneritr;
		TsInt N = shapes[0];
		TsInt D = shapes[1];
