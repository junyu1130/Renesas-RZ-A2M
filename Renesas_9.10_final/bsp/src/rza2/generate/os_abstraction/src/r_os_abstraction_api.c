...
#define R_OS_PRV_HUGE_STACK_SIZE_X2    (configDEFAULT_STACK_SIZE*8)
...
void (*p_main_task_func_t)(void) = NULL;
...
{
	static bool_t bInitialized = FALSE;

	if (bInitialized == TRUE)
		return;
    /* Heap_5 method of memory management */
    /* should define xHeapRegions in application code */
	configASSERT(xHeapRegions[0].xSizeInBytes != 0);
...
/**
 *	@brief	   Register Application main task callback. It's an optional function. To enable this function, 
 *				   call this API before calling R_OS_KernelInit() 
 *	@param[in] p_main_task_func the callback task object.
 *	@retval    NONE.
*/
	void R_OS_RegistrMainTaskCb (void (*p_main_task_func)(void))
{
	p_main_task_func_t = p_main_task_func;
}
/**********************************************************************************************************************
 End of function R_OS_RegistrMainTaskCb
 *********************************************************************************************************************/
        ...
        int_t priority)
{
	...
	case R_OS_ABSTRACTION_HUGE_STACK_SIZE_X2:
	{
            /* Stack size is defined in FreeRTOS as unsigned short, convert to 32 bit */
            actual_stack = (uint32_t) R_OS_PRV_HUGE_STACK_SIZE_X2;
            break;	
	}
        default:
        {
...
//            xSemaphoreGiveFromISR((SemaphoreHandle_t ) ( *p_semaphore), NULL);
        	xSemaphoreGiveFromISR((SemaphoreHandle_t ) ( *p_semaphore), &xHigherPriorityTaskWoken);
 ...
 **********************************************************************************************************************/
 #if (0)
 //User the Wrapper API in os_port_freertos.c
/**********************************************************************************************************************
