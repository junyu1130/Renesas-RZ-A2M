...
/**
 * @brief R_RIIC_ScInitChannel initialises a channel for set-up
 *
 * This function configures device registers in preparation for
 * configuration.
 *
 * @param[in] Channel to initialise.
 *
 * @pclk_frequency[in] PCLK frequency for RIIC.
 *
 * @retval DRV_SUCCESS Successful operation.
 * @retval DRV_ERROR Combination of frequency setting does not conform to the standard.
 */
int_t R_RIIC_ScInitChannel (int_t channel, uint32_t pclk_frequency)
{
    ...
    {
        /* check that open the open target channel is registered in the SC table */
        for (sc_index = 0; sc_index < sc_table_size; sc_index++)
        {
            if (RIIC_SC_TABLE[sc_index].channel == channel)
            {
                break;
            }
        }

        if (sc_table_size == sc_index)
        {
            retval = DRV_ERROR;
        }
    }
    ...
    {
        gs_riic_config[channel].riic_mode = RIIC_SC_TABLE[sc_index].config.riic_mode;
        for (slave_addr_num = 0; slave_addr_num < RIIC_SLAVE_ADDR_NUM_MAX; slave_addr_num++)
        {
            gs_riic_config[channel].slave_address_enable[slave_addr_num] =
                    RIIC_SC_TABLE[sc_index].config.slave_address_enable[slave_addr_num];
            gs_riic_config[channel].slave_address[slave_addr_num] =
                    RIIC_SC_TABLE[sc_index].config.slave_address[slave_addr_num];
            gs_riic_config[channel].slave_address_length[slave_addr_num] =
                    RIIC_SC_TABLE[sc_index].config.slave_address_length[slave_addr_num];
        }
        gs_riic_config[channel].frequency = RIIC_SC_TABLE[sc_index].config.frequency;
        gs_riic_config[channel].duty = RIIC_SC_TABLE[sc_index].config.duty;
        gs_riic_config[channel].rise_time = RIIC_SC_TABLE[sc_index].config.rise_time;
        gs_riic_config[channel].fall_time = RIIC_SC_TABLE[sc_index].config.fall_time;
        gs_riic_config[channel].noise_filter_stage = RIIC_SC_TABLE[sc_index].config.noise_filter_stage;
        gs_riic_config[channel].timeout = RIIC_SC_TABLE[sc_index].config.timeout;
        gs_riic_config[channel].format = RIIC_SC_TABLE[sc_index].config.format;
        gs_riic_config[channel].host_address_enabled = RIIC_SC_TABLE[sc_index].config.host_address_enabled;
        gs_riic_config[channel].tei_priority = RIIC_SC_TABLE[sc_index].config.tei_priority;
        gs_riic_config[channel].ri_priority = RIIC_SC_TABLE[sc_index].config.ri_priority;
        gs_riic_config[channel].ti_priority = RIIC_SC_TABLE[sc_index].config.ti_priority;
        gs_riic_config[channel].spi_priority = RIIC_SC_TABLE[sc_index].config.spi_priority;
        gs_riic_config[channel].sti_priority = RIIC_SC_TABLE[sc_index].config.sti_priority;
        gs_riic_config[channel].naki_priority = RIIC_SC_TABLE[sc_index].config.naki_priority;
        gs_riic_config[channel].ali_priority = RIIC_SC_TABLE[sc_index].config.ali_priority;
        gs_riic_config[channel].tmoi_priority = RIIC_SC_TABLE[sc_index].config.tmoi_priority;
...
/**
 * @brief R_RIIC_CloseChannel is to be called when closing a channel.
 *
 * This function clear device registers and shutdown.
 *
 * @param[in] Channel to close.
 *
 * @retval  none
 */
 ...
 * configuration.
 *
 * @param[in] Channel to configure.
 *
 * @*p_cfg[in] RIIC configure parameter.
 *
 * @retval DRV_SUCCESS Successful operation.
 * @retval DRV_ERROR Combination of frequency setting does not conform to the standard.
 */
int_t R_RIIC_SetConfig (int_t channel, st_riic_config_t *p_cfg)
{
    ...
    if (DRV_SUCCESS == retval)
    {
        ...
        for (slave_addr_num = 0; slave_addr_num < RIIC_SLAVE_ADDR_NUM_MAX; slave_addr_num++)
        {
            gs_riic_config[channel].slave_address_enable[slave_addr_num] = p_cfg->slave_address_enable[slave_addr_num];
            gs_riic_config[channel].slave_address[slave_addr_num] = p_cfg->slave_address[slave_addr_num];
            gs_riic_config[channel].slave_address_length[slave_addr_num] = p_cfg->slave_address_length[slave_addr_num];
        }
 ...
 * configuration.
 *
 * @param[in] Channel to getting configuration parameters.
 *
 * @*p_cfg[out] RIIC configure parameter.
 *
 * @retval DRV_SUCCESS Successful operation.
 * @retval DRV_ERROR Combination of frequency setting does not conform to the standard.
 */
...
/**
 * @brief R_RIIC_TransmitStop issues STOP condition.
 *
 * This function is called by HLD, and issues STOP condition.
 *
 * @param[in] Channel that issues STOP condition.
 *
 * @retval  none
 */
...
/**
 * @brief R_RIIC_ClearStop clear STOP condition bit.
 *
 * This function is called by HLD, and clear STOP condition bit.
 *
 * @param[in] Channel that clear STOP condition bit.
 *
 * @retval  none
 */
...
/**
 * @brief R_RIIC_TransmitStart issues START condition.
 *
 * This function is called by HLD, and issues START condition.
 *
 * @param[in] Channel that issues START condition.
 *
 * @retval  none
 */
...
/**
 * @brief R_RIIC_TransmitRestart issues RESTART condition.
 *
 * This function is called by HLD, and issues RESTART condition.
 *
 * @param[in] Channel that issues RESTART condition.
 *
 * @retval  none
 */
...
/**
 * @brief R_RIIC_ClearNack clear receiving NACK bit.
 *
 * This function is called by HLD, and clear receiving NACK bit.
 *
 * @param[in] Channel that clear receiving NACK bit.
 *
 * @retval  none
 */
...
/**
 * @brief  R_RIIC_GetAckStatus get ack status.
 *
 * This function is called by HLD, and issues stop condition.
 *
 * @param[in] Channel to get ack status.
 *
 * @retval  0: ACK received
 * @retval  1: NACK received
 */
...
/**
 * @brief R_RIIC_TransmitAck tranfer ACK.
 *
 * This function is called by HLD, and transfer ACK to write device.
 *
 * @param[in] Channel that transfer ACK.
 *
 * @retval  none
 */
...
/**
 * @brief R_RIIC_TransmitNack tranfer NACK.
 *
 * This function is called by HLD, and transfer NACK to write device.
 *
 * @param[in] Channel that transfer NACK.
 *
 * @retval  none
 */
 ...
 *
 * This function is called by HLD, and et detection status of STOP condition.
 *
 * @param[in] Channel that et detection status of STOP condition
 *
 ...
 *
 * @param[in] RIIC channel of waiting slave address
 *
...
/**
 * @brief R_RIIC_GetSlaveAddrNum get slave address number of detected slave address.
 *
 * This function is called by HLD, and get slave address number of detected slave address.
 *
 * @param[in] Channel that gets slave address number of detected slave address.
 *
 ...
 * @retval  DRV_ERROR: Slave address wasn't detected.
 */
 ...
 * Arguments    : pVerInfo[out]  version info
...
/**
 * @brief riic_adjust_scl_clk Adjust RIIC clock parameter
 * Error in frequency by rise_time and fall_time.
 * Therefore, adjust the length of high width and low width parameter.
 *
 * @p_cfg[in] RIIC frequency parameter.
 *
 * @retval  none
 */
static int_t riic_adjust_scl_clk (st_riic_clk_check_param_t *p_cfg)
{
    ...
    {
        /* cast to uint32_t */
        unit_time = ((RIIC_UNIT_TIME_SCL * RIIC_UNIT_PCLK_HZ) / p_cfg->pclk) * (uint32_t) (1 << (p_cfg->pclk_div));

        /* Calculate how much rise_time corresponds to unit of SCL width */
        width_delay = (p_cfg->rise_time / unit_time);

        /* casts to uint32_t */
        if ((p_cfg->high_width >= ((uint32_t) width_delay + p_cfg->noise_filter_stage))
                && (RIIC_BITRATE_REG_MAX >= (p_cfg->high_width - ((uint32_t) width_delay + p_cfg->noise_filter_stage))))
        {
    ...
    if (DRV_SUCCESS == retval)
    {
        ...
        /* casts to uint32_t */
        if ((p_cfg->low_width >= ((uint32_t) width_delay + p_cfg->noise_filter_stage))
                && (RIIC_BITRATE_REG_MAX >= (p_cfg->low_width - ((uint32_t) width_delay + p_cfg->noise_filter_stage))))
        {
            /* As the width widens by rise_time and filter steps, subtract them */
...
/******************************************************************************
 * End of Function riic_adjust_clk_param
 ******************************************************************************/

/**
 * @brief Reset the RIIC channel
 *
 * riic_set_reset resets an RIIC channel HW.
 *
 * @param[in] channel RIIC channel to be reset.
 *
...
/**
 * @brief Release reset the RIIC channel
 *
 * riic_release_reset Release resets an RIIC channel HW.
 *
 * @param[in] Channel RIIC channel to release resett.
 *
 * @retval  DRV_SUCCESS
 */
static int_t riic_release_reset (int_t channel)
...
/**
 * @brief Register slave address for the RIIC channel
 *
 * riic_register_slave_addrss Register slave address for the RIIC channel.
 * This function is called only when operating in slave mode.
 * The 3registered addresses are detected in parallel.
 *
 * @param[in] Channel The channel to resigter slave address .
 *
 * @*p_cfg[in] RIIC configure parameter.
 *
 * @retval  none
        ...
        for (addr_num = 0; addr_num < RIIC_SLAVE_ADDR_NUM_MAX; addr_num++)
        {
            if (false == p_cfg->slave_address_enable[addr_num])
            {
                ...
                switch (addr_num)
                {
                    ...
                    {
                        /* clear slave address */
                        RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSER.LONG, (uint32_t) 0, RIIC_ICSER_SAR0_SHIFT, RIIC_ICSER_SAR0);

                        /* cast to volatile uint32_t pointer */
                        RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSAR0.LONG, (uint32_t) 0, IOREG_NONSHIFT_ACCESS, IOREG_NONMASK_ACCESS);
                    }
                    break;
                    ...
                    {
                        /* clear slave address */
                        RZA_IO_RegWrite_32((volatile uint32t *) &gsp_riic[channel]->ICSER.LONG, (uint32_t) 0, RIIC_ICSER_SAR1_SHIFT, RIIC_ICSER_SAR1);

                        /* cast to volatile uint32_t pointer */
                        RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSAR1.LONG, (uint32_t) 0, IOREG_NONSHIFT_ACCESS, IOREG_NONMASK_ACCESS);
                    }
                    break;
                    ...
                    {
                        /* clear slave address */
                        RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSER.LONG, (uint32_t) 0, RIIC_ICSER_SAR2_SHIFT, RIIC_ICSER_SAR2);

                        /* cast to volatile uint32_t pointer */
                        RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSAR2.LONG, (uint32_t) 0, IOREG_NONSHIFT_ACCESS, IOREG_NONMASK_ACCESS);
                    }
                    break;
            ...
            else
            {
                if ((RIIC_DEVICE_ADDR_7 == p_cfg->slave_address_length[addr_num]))
                {
                    switch (addr_num)
                    {
                        ...
                        {
                            /* clear slave address */
                            RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSER.LONG, (uint32_t) 1, RIIC_ICSER_SAR0_SHIFT, RIIC_ICSER_SAR0);

                            /* cast to volatile uint32_t pointer */
                            RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSAR0.LONG, (uint32_t) 1, RIIC_ICSAR0_FS0_SHIFT, RIIC_ICSAR0_FS0);

                            /* cast to volatile uint32_t pointer */
                            RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSAR0.LONG, (uint32_t) p_cfg->slave_address[addr_num], RIIC_ICSAR0_SVA_SHIFT, RIIC_ICSAR0_SVA);
                        }
                        break;
                        ...
                        {
                            /* clear slave address */
                            RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSER.LONG, (uint32_t) 1, RIIC_ICSER_SAR1_SHIFT, RIIC_ICSER_SAR1);

                            /* cast to volatile uint32_t pointer */
                            RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSAR1.LONG, (uint32_t) 1, RIIC_ICSAR1_FS1_SHIFT, RIIC_ICSAR1_FS1);

                            /* cast to volatile uint32_t pointer */
                            RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSAR1.LONG, (uint32_t) p_cfg->slave_address[addr_num], RIIC_ICSAR1_SVA_SHIFT, RIIC_ICSAR1_SVA);
                        }
                        break;
                        ...
                        {
                            /* clear slave address */
                            RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSER.LONG, (uint32_t) 1, RIIC_ICSER_SAR2_SHIFT, RIIC_ICSER_SAR2);

                            /* cast to volatile uint32_t pointer */
                            RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSAR2.LONG, (uint32_t) 1, RIIC_ICSAR2_FS2_SHIFT, RIIC_ICSAR2_FS2);

                            /* cast to volatile uint32_t pointer */
                            RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSAR2.LONG, (uint32_t) p_cfg->slave_address[addr_num], RIIC_ICSAR2_SVA_SHIFT, RIIC_ICSAR2_SVA);
                        }
                        break;
                ...
                else
                {
                    switch (addr_num)
                    {
                        ...
                        case RIIC_SLAVE_ADDR_NUM_0:
                        {
                            ...
                            /* cast to volatile uint32_t pointer */
                            RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSAR0.LONG, (uint32_t) p_cfg->slave_address[addr_num], RIIC_ICSAR0_SVA0_SHIFT, RIIC_ICSAR0_SVA0);
                        }
                        break;
                        ...
                        case RIIC_SLAVE_ADDR_NUM_1:
                        {
                            ...
                            /* cast to volatile uint32_t pointer */
                            RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSAR1.LONG, (uint32_t) p_cfg->slave_address[addr_num], RIIC_ICSAR1_SVA0_SHIFT, RIIC_ICSAR1_SVA0);
                        }
                        break;
                        ...
                        case RIIC_SLAVE_ADDR_NUM_2:
                        {
                            ...
                            /* cast to volatile uint32_t pointer */
                            RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICSAR2.LONG, (uint32_t) p_cfg->slave_address[addr_num], RIIC_ICSAR2_SVA0_SHIFT, RIIC_ICSAR2_SVA0);
                        }
                        break;
...
/**
 * @brief riic_set_scl_clk Sets register of RIIC clock parameter
 * riic_set_scl_clk Sets RIIC SCL parameter.
 *
 * @param[in] Channel RIIC channel to set clock parameter.
 *
 * @*p_cfg[in] RIIC configure parameter.
 *
 * @retval DRV_SUCCESS Successful operation.
...
/**
 * @brief riic_set_noise_filter Sets register of RIIC noise filter
 *
 * riic_set_noise_filter Sets RIIC noise filter.
 *
 * @param[in] Channel RIIC channel to set noise filter.
 *
 * @*p_cfg[in] RIIC configure parameter.
 *
 * @retval  none
...
static int_t riic_set_noise_filter (int_t channel, st_riic_config_t *p_cfg)
{
    ...
    RZA_IO_RegWrite_32((volatile uint32_t *) &gsp_riic[channel]->ICMR3.LONG,
            (uint32_t) gs_noise_filter_nf_table[p_cfg->noise_filter_stage], RIIC_ICMR3_NF_SHIFT, RIIC_ICMR3_NF);
...
/**
 * @brief riic_set_timeout Sets register of RIIC timeout
 *
 * riic_set_timeout Sets RIIC timeout parameter.
 * The count-up permission setting is performed at the start of RIIC transfer.
 *
...
/**
 * @brief riic_set_bus_format Sets register of RIIC bus format
 *
 * riic_set_bus_format Sets RIIC bus format parameter.
 *
 * @param[in] Channel RIIC channel to set bus format.
 *
 * @*p_cfg[in] RIIC configure parameter.
 *
 * @retval  none
