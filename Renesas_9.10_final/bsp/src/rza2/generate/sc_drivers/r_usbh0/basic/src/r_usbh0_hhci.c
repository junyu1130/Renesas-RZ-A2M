...
typedef struct
{
    ...
    uint32_t        mess_read;
    uint32_t        mess_cnt;
    uint32_t        data_buff[USBH0_HCI_MESS_MAX];
...
void r_usbh0_hstd_hci_task(void)
{
    ...
    if (p_task_info->mess_cnt)
    {
        ...
        data    = p_task_info->data_buff[p_task_info->mess_read];
        ...
        switch(message)
        {
            ...
            case USBH0_HCI_MESS_OHCI_INT:
                USBH0_HCI_PRINTF1("USBH0_HCI_MESS_OHCI_INT : %08x \n", data);
                usbh0_hstd_OhciMainRoutine(data);
            break;
...
void usbh0_hstd_hci_interrupt_handler(void)
{
    ...
    if( (USB00.HCINTERRUPTSTATUS.BIT.RHSC && USB00.HCINTERRUPTENABLE.BIT.RHSCE) || (USB00.USBSTS.BIT.PortChangeDetect && USB00.USBINTR.BIT.PortChangeDetectEnable) )
    {
 ...
 * Note         :
***********************************************************************************************************************/
void usbh0_hstd_hci_send_message_from_int(int32_t task_no, int8_t message, uint32_t data)
{
    st_usbh0_hci_task_info_t    *p_task_info;
    ...
    p_task_info->data_buff[p_task_info->mess_write] = data;
...
******************************************************************************/
uint32_t r_usbh0_pa_to_va(uint32_t vaddr)
{
    if (0x80300000 == (vaddr & 0x80300000))
    {
        vaddr |= 0x02000000;
    }
