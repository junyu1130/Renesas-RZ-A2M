...
/*******************************************************************************
* File: jcu_pl.c
*    JPEG Codec Unit (JCU) Sample Driver. OS Porting Layer. For RZ/A2M.
*
* - $ModuleRevision: {Masters}\JCU\RZ2\Single\24_RZA2_en $
* - $Rev: $
* - $Date: 2018-11-28T17:13:30+09:00 $
******************************************************************************/
...
errnum_t  R_JCU_OnInitialize(void)
{
	...
	er= R_INTC_RegistIntFunc( INTC_ID_JPGC_JEDI,  (r_co_intc_function_t) &( JCU_IRQ_JEDI_Handler ) );  /* MISRA 16.9 */
		if ( er != INTC_SUCCESS ) { e=JCU_ERROR_PARAM_61; goto fin; }  /* Error: RegistIntFunc */

	er= R_INTC_SetPriority( INTC_ID_JPGC_JEDI,  JCU_INT_PRIORITY );
		if ( er != INTC_SUCCESS ) { e=JCU_ERROR_PARAM_62; goto fin; }  /* Error: IntPriority */


	/* Register "JDTI" */
	er= R_INTC_RegistIntFunc( INTC_ID_JPGC_JDTI,  (r_co_intc_function_t) &( JCU_IRQ_JDTI_Handler ) );  /* MISRA 16.9 */
		if ( er != INTC_SUCCESS ) { e=JCU_ERROR_PARAM_63; goto fin; }  /* Error: RegistIntFunc */

	er= R_INTC_SetPriority( INTC_ID_JPGC_JDTI,  JCU_INT_PRIORITY );
		if ( er != INTC_SUCCESS ) { e=JCU_ERROR_PARAM_64; goto fin; }  /* Error: IntPriority */
...
bool_t  R_JCU_DisableInterrupt(void)
{
	...
	er= R_INTC_Disable( (uint16_t) INTC_ID_JPGC_JEDI );
		if ( er != INTC_SUCCESS ) { oblivious = __LINE__; }

	er= R_INTC_Disable( (uint16_t) INTC_ID_JPGC_JDTI );
		if ( er != INTC_SUCCESS ) { oblivious = __LINE__; }

	if ( oblivious != 0 ) { e=E_JCU_OBLIVIOUS_1; goto fin; }  /* Error: */
	e=0;
fin:
	R_ERROR_Set( e );
...
void  R_JCU_EnableInterrupt(void)
{
	...
	if ( oblivious != 0 ) { e=E_JCU_OBLIVIOUS_2; goto fin; }  /* Error: */
	e=0;
fin:
	R_ERROR_Set( e );
}
