 ...
 ******************************************************************************/
extern void R_VDC_OnInitialize ( const uint32_t user_num );
extern void R_VDC_OnFinalize( const uint32_t user_num );
...
vdc_error_t R_RVAPI_GraphCreateSurfaceVDC(const vdc_channel_t ch, const gr_surface_disp_config_t * const gr_disp_cnf)
{
    ...
    else
    {
        ...
        /* Display area */
        read_cfg.gr_grc.vs = (uint16_t) (gr_disp_cnf->disp_area.vs_rel + full_screen.vs);  /* vertical start position */
        read_cfg.gr_grc.vw = (uint16_t) gr_disp_cnf->disp_area.vw_rel;                     /* vertical display size */
        read_cfg.gr_grc.hs = (uint16_t) (gr_disp_cnf->disp_area.hs_rel + full_screen.hs);  /* horizontal start position */
        read_cfg.gr_grc.hw = (uint16_t) gr_disp_cnf->disp_area.hw_rel;                     /* horizontal display size */
    ...
    else
    {
        ...
        if (NULL != v_disp_cnf)
        {
            ...
            if (VDC_OK == error)
            {
                ...
                /* Display area */
                read_cfg.gr_grc.vs = (uint16_t)(v_disp_cnf->disp_area.vs_rel + full_screen.vs);  /* vertical start position */
                read_cfg.gr_grc.vw = v_disp_cnf->disp_area.vw_rel;                               /* vertical display size */
                read_cfg.gr_grc.hs = (uint16_t)(v_disp_cnf->disp_area.hs_rel + full_screen.hs);  /* horizontal start position */
                read_cfg.gr_grc.hw = v_disp_cnf->disp_area.hw_rel;                               /* horizontal display size */
