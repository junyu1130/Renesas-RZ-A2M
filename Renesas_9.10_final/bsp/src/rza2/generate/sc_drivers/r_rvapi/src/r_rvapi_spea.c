 ...
 ******************************************************************************/
vdc_error_t R_RVAPI_GraphCreateSurfaceSPEA(const vdc_channel_t ch, const gr_surface_disp_config_t * const gr_disp_cnf)
{
    vdc_error_t error;
    vdc_read_t spea_cnf;
    vdc_start_t start;
    ...
    else
    {
        ...
        spea_cnf.adj_sel        = VDC_ON;                          /* Measures to decrease the influence by folding pixels/lines (on/off) */
        spea_cnf.gr_format      = VDC_GR_FORMAT_ARGB8888;          /* Format of the frame buffer read signal */
        spea_cnf.gr_ycc_swap    = VDC_GR_YCCSWAP_CBY0CRY1;         /* Controls swapping of data read from buffer in the YCbCr422 format */
 ...
 ******************************************************************************/
spea_error_t R_RVAPI_WindowOffsetSPEA(const vdc_layer_id_t layer_id, const uint16_t offset_x,
 ...
 * @retval      VDC driver error code
 ******************************************************************************/
vdc_error_t R_RVAPI_GraphCreateSurfaceRLE(const vdc_channel_t ch, const gr_surface_disp_config_t * const gr_disp_cnf)
{
    vdc_error_t error;
    vdc_read_t spea_cnf;
    vdc_start_t start;
    ...
    else
    {
        ...
        spea_cnf.gr_ln_off      = ((uint16_t)gr_disp_cnf->disp_area.hw_rel * RLE_STRIDE);       /* Frame buffer line offset address */
        spea_cnf.width_read_fb  = NULL;                                                         /* Width of the image read from frame buffer */
        spea_cnf.adj_sel        = VDC_ON;                                                       /* Measures to decrease the influence by folding pixels/lines (on/off) */
