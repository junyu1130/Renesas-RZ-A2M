...
static void msleep_ov7740 (uint32_t wait);
...
void R_CAMERA_Ov7740Init (int videoResolution)
{
    ...
    msleep_ov7740(10);                  // The Omnivision chip needs some wait time after reset
...
static void write_omni_table (const omni_register_t Table[])
{
    ...
    {
        R_RIIC_CameraWrite(DEVDRV_CH_1, OV7740_I2C_ADDR, Table[i].addr, Table[i].val);
    }
}
/*****************************************************************************
