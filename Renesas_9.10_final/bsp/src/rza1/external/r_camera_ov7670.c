...
static void write_omni_table (const omniregister_t Table[])
{
    ...
    {
        R_RIIC_CameraWrite(DEVDRV_CH_1, OV7670_I2C_ADDR, Table[i].addr, Table[i].val);
    }
}
...
/***********************************************************************************************************************
 * Function Name: msleep_ov7670
 * Description  : Simple spin loop to provide a delay. This should be replaced by an OS wait/delay call if available.
 * Arguments    : wait - number of loops to spin.
 * Return Value : void
 ***********************************************************************************************************************/
static void msleep_ov7670 (uint32_t wait)
{
    uint32_t i;
    for (i = WAIT_TICK; 0 != wait; i--)
    {
        if (0 == i)
        {
            wait--;
...
void R_CAMERA_Ov7670Init (int videoResolution)
{
    ...
    R_RIIC_CameraWrite(DEVDRV_CH_1, OV7670_I2C_ADDR, REG_COM7, COM7_RESET);
    msleep_ov7670(100);

    /* Release reset and wait a while */
    R_RIIC_CameraWrite(DEVDRV_CH_1, OV7670_I2C_ADDR, REG_COM7, COM7_FMT_VGA);
    msleep_ov7670(100);
