...
*******************************************************************************/
/**************************************************************************//**
* @file         r_vdc5_l_check_parameter.c
* @version      1.00
* $Rev: 99 $
* $Date:: 2017-04-10 20:56:26 +0900#$
* @brief        RZ/A1L VDC5 driver parameter check processing
******************************************************************************/
...
#include    "r_vdc5_l_check_parameter.h"
...
vdc5_error_t VDC5_CallbackISRCheckPrm (const vdc5_channel_t ch, const vdc5_int_t * const param)
{
    ...
    if (param->callback != 0)
    {
        if ((param->type == VDC5_INT_TYPE_VLINE) || (param->type == VDC5_INT_TYPE_S0_WLINE))
        {
            /* Line interrupt set */
    ...
    const vdc5_read_t   * const param)
{
    ...
    if ((param->gr_flm_sel != VDC5_GR_FLM_SEL_SCALE_DOWN) &&
        (param->gr_flm_sel != VDC5_GR_FLM_SEL_FLM_NUM) &&
        (param->gr_flm_sel != VDC5_GR_FLM_SEL_POINTER_BUFF))
    {
        error = VDC5_ERR_PARAM_UNDEFINED;
        goto END;
    }
    ...
    {
        btm_base_tmp    = VDC5_ShrdPrmGetFrBuffBtm();
    ...
    else    /* VDC5_GR_TYPE_GR2 or VDC5_GR_TYPE_GR3 */
    {
    ...
    const vdc5_alpha_blending_t * const param)
{
    ...
    if ((layer_id != VDC5_LAYER_ID_2_RD) && (layer_id != VDC5_LAYER_ID_3_RD))
    {
        error = VDC5_ERR_PARAM_LAYER_ID;
        goto END;
    }
    ...
    const vdc5_alpha_blending_rect_t    * const param)
{
    ...
    if (param != NULL)
    {
        ...
        if (param->alpha_rect != NULL)
        {
            ...
            if ((param->alpha_rect->gr_arc_coef < (int16_t)VDC5_CPARA_GR_ARC_COEF_MIN) ||
                (param->alpha_rect->gr_arc_coef > (int16_t)VDC5_CPARA_GR_ARC_COEF_MAX))
            {
                error = VDC5_ERR_PARAM_EXCEED_RANGE;
                goto END;
            }
...
static vdc5_error_t CheckGraphicsTypeLayerId (const vdc5_layer_id_t layer_id)
{
    ...
    if ((layer_id != VDC5_LAYER_ID_0_RD) && (layer_id != VDC5_LAYER_ID_2_RD) && (layer_id != VDC5_LAYER_ID_3_RD))
    {
        error = VDC5_ERR_PARAM_LAYER_ID;
        goto END;
    }
