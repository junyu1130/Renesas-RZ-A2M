...
void (*g_intc_func_table[INTC_ID_TOTAL]) (uint32_t int_sense) =
{
 ...
 *              : INTC interrupt handler function table g_intc_func_table[].
 ...
 *              : interrupt handler function table g_intc_func_table[] is executed.
 * Arguments    : uint16_t int_id    : Interrupt ID
...
{
    if (g_intc_func_table[int_id] != userdef_intc_dummy_interrupt)
    {
        /* Call the function installed in the array of installed handler functions. */
        g_intc_func_table[int_id](int_sense);
    }
    else
    {
        /* Call the default interrupt handler, pass the interrupt id as the sense parameter */
        g_intc_func_table[int_id](int_id);
    }
}
/*******************************************************************************
