...
#include "../inc/devdrv_ostm.h"            /* OSTM Driver header */
#include "../../intc/inc/devdrv_intc.h"            /* INTC Driver Header */
#include "iodefine.h"
//#include "main.h"
...
#define MAX_CYCLE_msec          (0xFFFFFFFF / configPERIPHERAL_CLOCK_HZ)
...
int32_t Userdef_OSTM0_Init(uint32_t mode, uint32_t cycle)
{
    ...
    OSTM0.OSTMnCMP = ((uint32_t)(configPERIPHERAL_CLOCK_HZ * cycle) / 1000 );
        /* Set timer in the cycle specified by the cycle */
        /* CMP = P0 clock (kHz) * cycle (msec)           */
        /* (Assume that P0 clock has 33.325MHz)          */

    /* ==== Register OS timer interrupt handler ==== */
    R_INTC_RegistIntFunc(INTC_ID_OSTM0TINT, ( ISR_FUNCTION )FreeRTOS_Tick_Handler);
