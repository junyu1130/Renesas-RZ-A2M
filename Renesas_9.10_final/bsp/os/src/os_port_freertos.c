 ...
 **/
void osSwitchTaskISR(BaseType_t pxHigherPriorityTaskWoken)
{
...
bool_t osAcquireMutex(OsMutex *mutex, systime_t timeout)
{
	portBASE_TYPE ret;
   ...
   if(timeout == INFINITE_DELAY)
   {
	  //Infinite timeout period
	   ...
	   ret = xSemaphoreTake(mutex->handle, portMAX_DELAY);
   }
   else
   {
	   ret = xSemaphoreTake(mutex->handle, OS_MS_TO_SYSTICKS(timeout));
   }
...
bool_t osCreateQueue(OsQueue *queue, uint_t length, uint_t size)
{
   //Create a queue object
	queue->handle = xQueueCreate(length, size);
...
bool_t osReceiveQueue(OsQueue *queue, void * const pvItemToQueue, systime_t timeout)
{
   portBASE_TYPE ret;
   ...
   {
	  //Infinite timeout period
	  ret = xQueueReceive(queue->handle, pvItemToQueue, portMAX_DELAY);
   }
   else
   {
	  //Wait for the specified time interval
	  ret = xQueueReceive(queue->handle, pvItemToQueue, OS_MS_TO_SYSTICKS(timeout));
   }
...
bool_t osSendQueue(OsQueue *queue, const void * const pvItemToQueue, systime_t timeout)
{
   portBASE_TYPE ret;
   ...
   {
	  //Infinite timeout period
	  ret = xQueueSend(queue->handle, pvItemToQueue, portMAX_DELAY);
   }
   else
   {
	  //Wait for the specified time interval
	  ret = xQueueSend(queue->handle, pvItemToQueue, OS_MS_TO_SYSTICKS(timeout));
   }
...
bool_t osReceiveQueueISR(OsQueue *queue, void * const pvItemToQueue, BaseType_t * const pxHigherPriorityTaskWoken)
{
	portBASE_TYPE ret;

	ret = xQueueReceiveFromISR(queue->handle, pvItemToQueue, pxHigherPriorityTaskWoken);

	return ret;
}

bool_t osSendQueueISR(OsQueue *queue, const void * const pvItemToQueue, BaseType_t * const pxHigherPriorityTaskWoken)
{
	portBASE_TYPE ret;

	ret = xQueueSendFromISR(queue->handle, pvItemToQueue, pxHigherPriorityTaskWoken);
